            ...
            case 'detail-events':
                ...
                foreach ($tables as $table) {
                    ...
                    if ($entry == $title) {
                        ...
                        $link = 'http://www.unisg.ch/UeberUns/Services/Sport/Sportangebot/';

                        foreach ($allRows as $row) {
                            if ($innerIndex > $firstRow) {
                                $allCells = $row->getElementsByTagName("td");
                                $linkUrl = '';

                                if ($title != "Schweizer Hochschulmeisterschaften") {
                                    $linkUrl = $link . $allCells->item(0)->getElementsByTagName('a')->item(0)->getAttribute('href');
                                } else{
                                    $linkUrl =$link . "Veranstaltungen.aspx";
                                }
                                $events[] = array(
                                    'title' => $allCells->item(0)->nodeValue,
                                    'subtitle' => $allCells->item(1)->nodeValue,
                                    'url' => $linkUrl
                                );
                            }
                            $innerIndex++;
