            ...
            case 'Imperial':
                ...
                if ($miles < 0.1) {
                    ...
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_FEET', number_format($feet, 0));
                } elseif ($miles < 15) {
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_MILES', number_format($miles, 1));
                } else {
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_MILES', number_format($miles, 0));
                }
                break;
            case 'Metric':
            default:
                if ($meters < 100) {
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_METERS', number_format($meters, 0));
                } elseif ($meters < 15000) {
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_KILOMETERS', number_format($meters / 1000, 1));
                } else {
                    $result = $this->getLocalizedString(
                            'DISTANCE_IN_KILOMETERS', number_format($meters / 1000, 0));
                }
    ...
    protected function generateTabForKey($tabKey, $feature, $dataController, &$tabJavascripts) {
        switch ($tabKey) {
            ...
            case 'rooms': {
                    ...
                    //describing room-links
                    $title = $feature->getTitle(); //getting building number
                    $fields = $feature->getFields(); //getting info-field of googlemaps
                    $details = current(array_values($fields)); //building
                    $floors[] = array(); // array
                    $floors = explode(',', $details); // separating the field values
                    $hasFloors = false; // boolean to check if the building has floor info

                    if (strlen(trim($floors[0])) > 0) {
                        $hasFloors = true; // do show rooms tab
                    } else {
                        $hasFloors = false; // do not show rooms tab
                    }

                    if (count($floors) > 0) {
                        //setting up rooms navlist 
                        foreach ($floors as $floor) {
                            $allFloors[] = array(
                                'title' => $floor,
                                'url' => $this->buildBreadcrumbURL('floor', array(
                                    'title' => $title, 'floorid' => $floor
                                        )
                                )
                            );
                        }

                        $this->assign('details', $allFloors);
                    }

                    return $hasFloors;
                }
            case 'campus': {
                    // show always the campus map
                    return true;
                }
            default:
                break;
    ...
    protected function initializeForPage() {
        ...
        switch ($this->page) {
            ...
            case 'floor':
                ...
                $buildingId = $this->getArg('title');
                $this->setPageTitle($buildingId . '-' . $floorId);

                // set image directory
                $imageDirectory = "/modules/map/images/rooms/";
                ...
                // build url to go back to building view

                $category = 'c5db3ceaeb';

                $buildingUrl = $this->buildURL('category', array(
                    'category' => $category
                        ));

                $places = array();
                $places[] = array(
                    'title' => 'Zur Gebäudeübersicht',
                    'url' => $buildingUrl
                );
