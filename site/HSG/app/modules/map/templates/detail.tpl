  ...
  {capture name="mapPane" assign="mapPane"}
    {block name="mapImage"}
      ...
      <div id="mapimage" class="image">
        ...
        {/if}
          <!-- Bild immer dahinter legen-->
          <img id="staticmapimage" src="/modules/map/images/defaultmap.png" width="300" height="300" alt="Map" />
      </div>
...
{/if}
{if in_array('campus', $tabKeys)}
  {capture name="campusPane" assign="campusPane"}
    <img width="100%" src="/modules/map/images/HSG-Campusplan.jpeg"/>
  {/capture}
  {$tabBodies['campus'] = $campusPane}
{/if}
{if in_array('rooms', $tabKeys)}
  {capture name="roomsPane" assign="roomsPane"}
      {include file="findInclude:common/templates/navlist.tpl" navlistItems=$details boldLabels=true listItemTemplateFile="findInclude:modules/$moduleID/templates/listItemWithID.tpl"}
  {/capture}
  {$tabBodies['rooms'] = $roomsPane}
{/if}
